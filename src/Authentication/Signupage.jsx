...
const SignUpPage = ({ setDropdownOpen, signedUp, setSignedUp }) => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        setSignedUp(true);
        setDropdownOpen(false); // Open dropdown on successful login
        navigate("/chatopening"); // Ensure this matches a valid route
      }
    });

    return () => unsubscribe();
  }, [setSignedUp, setDropdownOpen, navigate]);
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2️ Store user data in Firestore (WITHOUT password)
      const userData = {
        uid: user.uid,
        username,
        name,
